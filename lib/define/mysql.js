...
module.exports.Context = function (config) {
    ...
    // 使用传进的config生成md5 对mysql连接池进行标记
    ...
    // 调用getConnection时，从连接池中取出连接，封装成MysqlConnection对象返回
...
function MysqlConnection(pool, readonly) {
    ...
    const that = this
    // 从连接池中获取连接conn，并赋给_conn成员，并标志_connected=true，发送'connected'事件，说明成功连接，生成日志。
...
// TODO: log
// 释放连接，先标记连接释放，再释放连接，并解除连接池的引用释放内存
...
// 只读查询，最后一个参数为callback
MysqlConnection.prototype.queryReadonly = function (sql, arg2, arg3) {
    // 如果没有连接上数据库，则会调用_init方法，直到监听到'connected'事件才会返回来重新调用queryReadonly方法
...
// TODO
// 删除，callback: error, results, fields
...
MysqlConnection.prototype.timeout = function (timeout) {
    $.ALL_METHODS.forEach(function (method) {
        MysqlConnection.prototype[method] = setTimeout(MysqlConnection.prototype[method], timeout)
    })
}
